"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const hardhat_1 = require("hardhat");
const factoryDeployerAccount = '0xBb6e024b9cFFACB947A71991E386681B1Cd1477D';
const minimumDeployerBalance = hardhat_1.ethers.parseEther('0.0247');
async function main() {
    const [signer] = await hardhat_1.ethers.getSigners();
    // Check deployer balance
    const deployerBalance = await hardhat_1.ethers.provider.getBalance(factoryDeployerAccount);
    if (deployerBalance < minimumDeployerBalance) {
        // Fund deployer
        console.log('Funding deployer...', {
            deployerBalance,
            minimumDeployerBalance,
        });
        const fundTx = await signer.sendTransaction({
            to: factoryDeployerAccount,
            value: minimumDeployerBalance - deployerBalance,
        });
        await fundTx.wait();
    }
    else {
        console.log('Deployer has enough funds', { deployerBalance });
    }
    console.log('Deploying EIP-2470...');
    // Deploy EIP-2470
    const txHash = await hardhat_1.ethers.provider.send('eth_sendRawTransaction', [
        '',
    ]);
    console.log('EIP-2470 deployed! Transaction hash: ', txHash);
}
main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
});
