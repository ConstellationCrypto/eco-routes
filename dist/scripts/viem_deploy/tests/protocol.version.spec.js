...
describe('ProtocolVersion Tests', () => {
    ...
    describe('on constructor', () => {
        it('should set the version to that given arg', () => {
            let pv = new ProtocolVersion_1.ProtocolVersion('1.0.0');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('latest');
            pv = new ProtocolVersion_1.ProtocolVersion('1.0.0-latest');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('latest');
            pv = new ProtocolVersion_1.ProtocolVersion('1.0.0-latest');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('latest');
            pv = new ProtocolVersion_1.ProtocolVersion('1.0.0-beta');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('beta');
            pv = new ProtocolVersion_1.ProtocolVersion('v1.0.0-beta');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('beta');
            pv = new ProtocolVersion_1.ProtocolVersion('1.0.0-rc');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('rc');
            pv = new ProtocolVersion_1.ProtocolVersion('v1.0.0-rc');
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('rc');
        });
        it('should throw if given version arg that is invalid', () => {
            expect(() => new ProtocolVersion_1.ProtocolVersion('invalid')).toThrow('Invalid version: invalid');
            expect(() => new ProtocolVersion_1.ProtocolVersion('version1.2')).toThrow('Invalid version: version1.2');
        });
        it('should set the version to the tag to env if not given', () => {
            process.env.GITHUB_REF = 'refs/tags/v1.0.0';
            let pv = new ProtocolVersion_1.ProtocolVersion();
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('latest');
            process.env.GITHUB_REF = 'refs/tags/v1.0.0-beta';
            pv = new ProtocolVersion_1.ProtocolVersion();
            expect(pv.getVersion()).toEqual('1.0.0');
            expect(pv.getReleaseTag()).toEqual('beta');
        });
        it('should throw if version tag from env is invalid', () => {
            process.env.GITHUB_REF = 'v1.0.0';
            expect(() => new ProtocolVersion_1.ProtocolVersion()).toThrow('GITHUB_REF is not a tag');
        });
    });
    describe('on updateProtocolVersion', () => {
        it('should call the package and .sol file updates', () => {
            const spy = jest.spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'updateProjectVersion');
            const mockVersionSol = jest.fn();
            const mockUpdatePackage = jest.fn();
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'updateVersionInSolidityFiles')
                .mockImplementation(mockVersionSol);
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'updatePackageJsonVersion')
                .mockImplementation(mockUpdatePackage);
            const pv = new ProtocolVersion_1.ProtocolVersion('1.0.0');
    ...
    describe('on isPatchUpdate', () => {
        ...
        beforeEach(() => {
            pv = new ProtocolVersion_1.ProtocolVersion(versionString);
        });
        it('should return false if no published version for tag exists', async () => {
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'getPublishedVersion')
                .mockResolvedValue('');
        ...
        it('should throw is the release version is the same as this', async () => {
            ...
            await expect(async () => await pv.isPatchUpdate()).rejects.toThrow(`Version of git tag ${versionString} is the same as the current published version: ${versionString}`);
        });
    ...
    describe('on getNewChains', () => {
        ...
        it("should return all chains if it can't extract the package", async () => {
            ...
            expect(await pv.getNewChains()).toEqual(chains_1.DeployChains);
        });
    ...
    describe('on getDeployChains', () => {
        ...
        it('should throw if a patch update has no new chains', async () => {
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'isPatchUpdate')
                .mockResolvedValue(true);
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'getNewChains')
                .mockResolvedValue([]);
            await expect(async () => await pv.getDeployChains()).rejects.toThrow('No new chains to deploy for a patch update');
        });
        it('should call getNewChains if its a patch update', async () => {
            jest
                .spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'isPatchUpdate')
                .mockResolvedValue(true);
            const spy = jest.spyOn(ProtocolVersion_1.ProtocolVersion.prototype, 'getNewChains');
